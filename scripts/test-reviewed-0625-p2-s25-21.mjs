...
import reviewedSet from '../src/data/reviewedQuestionSets/cie-0625-0625_s25_qp_21.json' with { type: 'json' }
...
const paperId = CAMBRIDGE_0625_P2_S25_21_REVIEW_LEDGER.paperId
const ledgerPath = path.join(root, 'src', 'data', 'reviewedQuestionSets', 'cambridge-0625-p2-s25-21-review-ledger.js')
...
assert.equal(CAMBRIDGE_0625_P2_S25_21_REVIEW_LEDGER_SCHEMA_VERSION, 'cambridge-0625-p2-s25-21-review-ledger.v1')
...
assert.deepEqual(CAMBRIDGE_0625_P2_S25_21_REVIEW_LEDGER.questions.map((row) => row.questionNumber), Array.from({ length: 40 }, (_item, index) => index + 1))
...
const wavesSet = buildSyllabusPracticeSet({
  routeId: 'cie-0625-igcse-physics',
  syllabusTopicIds: ['0625-igcse-topic-03'],
  questionCount: 10,
  components: [2],
  seed: 20260815,
  questionBank: unifiedQuestionBank,
})
assert.equal(wavesSet.questionCount, 10)
assert.ok(wavesSet.questionGroups.every((question) => question.routeId === 'cie-0625-igcse-physics' && question.paperComponent === 2))

const reviewedIds = []
for (const row of CAMBRIDGE_0625_P2_S25_21_REVIEW_LEDGER.questions) {
  ...
  const incorrect = scorePaperMultipleChoice({ answer: { choice: row.correctOption === 'A' ? 'B' : 'A' }, answerKey: row.correctOption, marks: 1 })
  const correct = scorePaperMultipleChoice({ answer: { choice: row.correctOption }, answerKey: row.correctOption, marks: 1 })
  assert.equal(incorrect.awarded, 0, `${questionId}: wrong option must score zero`)
  assert.equal(correct.awarded, 1, `${questionId}: reviewed option must score one`)
...
assert.equal(canonical.length, 40, 'all reviewed 0625/21 groups must enter the canonical practice bank')
assert.ok(canonical.every((question) => question.routeId === 'cie-0625-igcse-physics' && question.stage === 'IGCSE'))
assert.ok(canonical.every((question) => question.paperComponent === 2 && question.answerType === 'multiple-choice' && question.answerKey))
