...
const assetRoot = path.join(root, 'public', 'question-assets')
...
const manifestPayload = {
  ...
  sourceIndexSha256: digest(fs.readFileSync(indexPath)),
  items: runtimeItems,
}
const manifest = {
  ...manifestPayload,
  checksum: digest(JSON.stringify(manifestPayload)),
}

if (writeManifest) {
  fs.writeFileSync(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, 'utf8')
  fs.writeFileSync(manifestIdentityPath, [
