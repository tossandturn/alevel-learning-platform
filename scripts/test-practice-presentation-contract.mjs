...
const paperCatalogHookSource = await import('node:fs').then(({ readFileSync }) => readFileSync(new URL('../src/hooks/usePaperCatalog.js', import.meta.url), 'utf8'))
...
assert.match(appSource, /const authoritativeTopicCtaPolicy[\s\S]{0,260}selectedTopicsMeetFormalFloor/, 'Topic Detail must derive CTA policy from the selected component review floor')
assert.doesNotMatch(appSource, /const studyReady = dynamicSyllabusRoute/, 'Topic Detail must not expose an under-review study-only set as a formal Topic Drill')
assert.match(appSource, /practiceReady \|\| sampleReady/, 'Topic Detail must render a start action only for an explicitly eligible practice mode')
assert.match(appSource, /data-topic-practice-cta=\{topicCtaPolicy\}/, 'Topic Drill CTA must expose its authoritative eligibility state to QA')
...
assert.match(paperWorkspaceSource, /const hasAuthoritativeCoachBinding = Boolean\(sharedIdentityToken\) && serverAttemptReady && Boolean\(coachQuestionPart\?\.id\)/, 'full-paper Coach must wait for a signed, persisted attempt and an authoritative reviewed question part')
assert.match(paperWorkspaceSource, /const coachAvailable = \(studyMode === 'past-paper-practice' \|\| submitted\) && hasAuthoritativeCoachBinding/, 'Past-paper practice must allow Coach before submit after authoritative binding, while Exam Simulation keeps the submit boundary')
assert.match(paperWorkspaceSource, /const coachQuestionPartId = coachQuestionPart\?\.id \|\| ''/, 'full-paper Coach must never fall back to a client-only answer-slot ID')
assert.match(paperAnswerSheetSource, /questionMetadataByNumber\[questionNumber\]\?\.parts\?\.some\(\(part\) => part\.id && part\.markingProvenance\)/, 'each full-paper Coach button must require its own authoritative part binding')
assert.match(paperCatalogHookSource, /options\.subject/, 'paper catalogs must support loading a target subject without changing the current route first')
assert.match(appSource, /paperCatalogState\.load\(\{ subject: bphoRoute\.subjectCode \}\)/, 'Coach agent actions must load the catalog for their target route')
