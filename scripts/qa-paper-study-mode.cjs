...
async function openPaper(page, baseUrl, mode) {
  ...
  const row = page.locator('.paper-table tbody tr').filter({ hasText: '0580_m25_qp_12.pdf' }).first()
...
async function run() {
  ...
  try {
    ...
    await page.locator('.paper-study-mode-note').waitFor()
    await page.locator('.paper-search input').fill('0580_m25_qp_12.pdf')
    await page.locator('.paper-table tbody tr').filter({ hasText: '0580_m25_qp_12.pdf' }).first().getByRole('button', { name: 'Open' }).click()
    await page.locator('.paper-workspace').waitFor()
    await page.waitForSelector('.pdf-canvas-scroll canvas')
