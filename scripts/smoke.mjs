...
assert.ok(appSource.includes('onClick={() => openPractice()}>Choose another topic</button>'), 'the home topic chooser must preserve the route selected in the preceding UI event')
assert.ok(!appSource.includes("openPractice({ subjectId: nextUnit?.subjectId || activeRoute.subjectId })}>Choose another topic"), 'the home topic chooser must not overwrite a freshly selected route with a stale render subject')
assert.ok(appSource.includes(": openPractice()}><PlayIcon />{nextUnit ?"), 'the primary choose-topic action must preserve a freshly selected route too')
...
const paperRouteEligibilitySource = fs.readFileSync(path.resolve(import.meta.dirname, '..', 'src', 'lib', 'paperRouteEligibility.js'), 'utf8')
assert.ok(paperLibrarySource.includes('paperItemMatchesActiveRoute(item, activeRoute)'), 'PaperLibrary must use the central route eligibility contract')
assert.ok(paperRouteEligibilitySource.includes("route?.stage === 'Competition'") && paperRouteEligibilitySource.includes("route?.stage === 'Admissions'"), 'specialist archives must bypass Cambridge component filtering')
