...
AI marking completes directly as `completed` with an immediately downloadable report; there is no teacher/examiner approval queue. `reviewRequired` remains in the structured result as a compatibility and uncertainty flag only. Fixed application copy labels uncertain, blurred, missing, or weakly referenced items and tells the student to add clearer material and retry; it does not introduce a mandatory human-review step. User instructions, evidence quotations, and structured AI text remain source-faithful after ordinary length/control-character formatting. Missing evidence still suppresses unsupported totals, and every result remains non-official and ineligible for formal progress.
