...
- `practicePolicy.schemaVersion=stem-topic-practice-policy-v1`: 6 distinct API-ready source groups for study, 12 reviewed groups per selected topic for formal practice; set sizes 6/10/15. `allowCrossTopicStudy=true` permits a multi-topic study-only set when the deduplicated union reaches 6 and every selected topic is represented by at least one API-ready source. It never makes formal progress eligible; count/list/start/rebind use the same distinction.
